...
public final class Sell {
    ...
    // 퀵판매
    public static void quickSellItem(Player player, ItemStack tempIS, String shopName, int tradeIdx, boolean isShiftClick, int slot) throws SQLException {
        ...
        EconomyResponse r = DynamicShop.getEconomy().depositPlayer(Bukkit.getServer().getOfflinePlayer(player.getUniqueId()), priceSum);
      //  XConomyAPI xcapi = new XConomyAPI();
     //   int r = xcapi.changebalance(player.getUniqueId(), player.getName(), BigDecimal.valueOf(priceSum), true);
       ...
       // if(r==0)
        if(r.transactionSuccess())
        {
            ...
            LogUtil.addsqlLog(shopName,tempIS.getType().toString(),-actualAmount,priceSum,"jobpoint",player);
            player.sendMessage(DynamicShop.dsPrefix + LangUtil.ccLang.get().getString("SELL_SUCCESS")
                    ...
                    .replace("{price}",econ.format(priceSum))
        ...
        {
            player.sendMessage(String.format("[Lien Economie: il y a eu une erreur: %s"+ r));
        }
    }

    // 판매
    public static void sellItemCash(Player player, String shopName, String tradeIdx, ItemStack tempIS, double priceSum, double deliverycharge, boolean infiniteStock) throws SQLException {
        double priceSellOld = DynaShopAPI.getSellPrice(shopName, tempIS);
        double priceBuyOld = Calc.getCurrentPrice(shopName,String.valueOf(tradeIdx),true);
        int stockOld = ShopUtil.ccShop.get().getInt(shopName+"." + tradeIdx + ".stock");
        // 상점에 돈이 없음
   ...
   //     if(r.transactionSuccess())
      ...
      //  int r = xcapi.changebalance(player.getUniqueId(), player.getName(), BigDecimal.valueOf(priceSum), true);
       ...
       // if(r==0)
...

        {
            ...
            //insertion en base
            LogUtil.addsqlLog(shopName,tempIS.getType().toString(),-actualAmount,priceSum,"vault",player);
    ...
    // 판매 jp
    public static void sellItemJobPoint(Player player, String shopName, String tradeIdx, ItemStack tempIS, double priceSum, double deliverycharge, boolean infiniteStock) throws SQLException {
        double priceSellOld = DynaShopAPI.getSellPrice(shopName, tempIS);
        double priceBuyOld = Calc.getCurrentPrice(shopName,String.valueOf(tradeIdx),true);
        int stockOld = ShopUtil.ccShop.get().getInt(shopName+"." + tradeIdx + ".stock");
        // 상점에 돈이 없음
